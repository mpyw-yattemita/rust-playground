...
impl<T> QuickSort<T> for [T] where T: Ord + Clone {
    ...
    fn quick_sort_by(&mut self, sort_by: fn(&T, &T) -> Ordering) {
        ...
        let left_min = 0;
        let right_max = self.len() - 1;
        let pivot = self[(left_min + right_max) >> 1].clone();
        ...
        loop {
            ...
            while left_cursor < right_max && sort_by(&self[left_cursor], &pivot).is_lt() {
                left_cursor += 1;
            }
            // 境界値「超過」の間，右カーソルが左側に進む
            while left_min < right_cursor && sort_by(&pivot, &self[right_cursor]).is_lt() {
                right_cursor -= 1;
            }
            ...
            // 入れ替えてからカーソルを次に進める
            self.swap(left_cursor, right_cursor);
        ...
        // 境界値「未満」の集合に対して再帰
        self[..left_cursor].quick_sort_by(sort_by);
        // 境界値「超過」の集合に対して再帰
        self[right_cursor + 1..].quick_sort_by(sort_by);
    }
}
